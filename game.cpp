...
void Game::gameStart()
{
	...
	block = new Block();
	nextBlock = new Block();
	nextText = new Text("font/novem___.ttf", "NEXT", 40, 255, 255, 255);

	nextDst.h = nextText->getHeight();
	nextDst.w = nextText->getWidth();
	nextDst.x = 3 * SCREEN_WIDTH / 3.5 - nextDst.w;
	nextDst.y = SCREEN_HEIGHT /5;

	scoreText = new Text("font/novem___.ttf", "SCORE: ", 30, 255, 255, 255);
	scoreDst.h = scoreText->getHeight();
	scoreDst.w = scoreText->getWidth();
	scoreDst.x = 3 * SCREEN_WIDTH / 3.5 - scoreDst.w;
	scoreDst.y = SCREEN_HEIGHT / 5 + SCREEN_HEIGHT / 2;

	levelText = new Text("font/novem___.ttf", "LEVEL: ", 30, 255, 255, 255);
	levelDst.h = levelText->getHeight();
	levelDst.w = levelText->getWidth();
	levelDst.x = 3 * SCREEN_WIDTH / 3.5 - scoreDst.w;
	levelDst.y = SCREEN_HEIGHT / 5 + SCREEN_HEIGHT / 3 + levelDst.h;
...
void Game::updateScore()
{
	...
	scoreNumberText = new Text("font/novem___.ttf", str, 30, 255, 255, 255);
	scoreNumberDst.h = scoreNumberText->getHeight();
	scoreNumberDst.w = scoreNumberText->getWidth();
	scoreNumberDst.x = 3 * SCREEN_WIDTH / 3.5 - scoreNumberDst.w + 9 * scoreNumberDst.w / 10;
	scoreNumberDst.y = SCREEN_HEIGHT / 5 + SCREEN_HEIGHT / 2;

	/*if (gameScore >= highScore) {
		highScore = gameScore;
	}
	//high score number
	std::string buff = std::to_string(highScore);

	highScoreText = new Text("font/novem___.ttf", buff, 30, 255, 255, 255);
	highScoreDst.h = highScoreText->getHeight();
	highScoreDst.w = highScoreText->getWidth();
	highScoreDst.x = 3 * SCREEN_WIDTH / 3.5 - highScoreDst.w + 9 * highScoreDst.w / 10;
	highScoreDst.y = SCREEN_HEIGHT / 4 + SCREEN_HEIGHT / 2;
	*/
...
void Game::updateLevel()
{
	...
	levelNumberText = new Text("font/novem___.ttf", str, 30, 255, 255, 255);
	levelNumberDst.h = levelNumberText->getHeight();
	levelNumberDst.w = levelNumberText->getWidth();
	levelNumberDst.x = 3 * SCREEN_WIDTH / 3.5 - levelNumberDst.w + 9 * levelNumberDst.w / 10;
	levelNumberDst.y = SCREEN_HEIGHT / 5 + SCREEN_HEIGHT / 3 + levelNumberDst.h;
}
...
void Game::render()
{
	...
	//highScoreText->render(&highScoreDst);
}
...
void Game::PlayBGM()
{
	...
	if (SDL_AudioInit(audioDriver) < 0)
		std :: cout << "Fail to initialize audio driver. SDL Error: " << SDL_GetError() << std :: endl;
}
